...
# ---------------- SimpleProtobuf Class (from bame.py/blacklist.py) ---------------- #
...
def process_ban(access_token):
    try:
        ...
        try:
            ...
            if response:
                return {"success": True, "message": "Done ban", "data": response.hex()}
            else:
...
# ---------------- BLACKLIST Function (from blacklist.py) ---------------- #
def process_blacklist(access_token):
    try:
        # Step 1: Inspect token
        inspect_url = f"https://100067.connect.garena.com/oauth/token/inspect?token={access_token}"
        inspect_headers = {
        ...
        try:
            ...
            response = send_once(online_ip, online_port, payload_bytes, recv_timeout=5.0)    
            if response:
                return {"success": True, "message": "Done blacklist", "data": response.hex()}
            else:
                return {"success": False, "message": "No response from server"}
        except Exception as e:
            return {"success": False, "message": f"Connection error: {str(e)}"}
...
def home():
    return jsonify({
        "message": "FreeFire Ban/Blacklist API",
        "endpoints": {
            "ban": "/ban?access_token={token}",
            "blacklist": "/blacklist?access_token={token}"
        },
        "note": "Both endpoints return responses in curly brackets {}"
    })
...
def blacklist_endpoint():
    ...
    result = process_blacklist(access_token)
    return jsonify(result)
